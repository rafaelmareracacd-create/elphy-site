...
import { contaGabinete, GR_GABINETE, INICIAL_GABINETE } from "@/lib/ferramentas/gabinete";
import { useFerramenta } from "./useFerramenta";
import Link from "next/link";
import { Fragment } from "react";
import MenuBotao from "./Menu";

/* Transcrito de docs/design-1.0/Gabinete-Mobile.dc.html */
export default function Gabinete() {
  const { grupos, linhas, impresso, numero, cupom, data, rotulo, pre, mostrado, unidade, sub, nota, seloTopo, seloMeio, seloBase, seloFundo, ledCor, ledTexto, brilho, codigo, barras, mandarTexto, mandar } =
    useFerramenta(GR_GABINETE, INICIAL_GABINETE, contaGabinete, "/ferramentas/gabinete/");
  return (
    <div className="dz-col" style={{ position: 'relative', width: '100%', minHeight: '2160px', overflow: 'hidden', display: 'flex', flexDirection: 'column', background: '#050607', color: '#eef2ee', fontFamily: 'var(--fb)' }}>
      ...
      <div style={{ padding: '28px 24px 0' }}>
        ...
        <p style={{ margin: '14px 0 0', fontSize: '16px', lineHeight: '24px', color: '#b9c1bb' }}>
          Quatro números de ficha, dois de cada lado. A nota diz quantos milímetros sobram — ou faltam.
        </p>
      </div>
      {grupos.map((g, i) => (
        <Fragment key={i}>
      ...
      <div style={{ margin: '28px 24px 0', boxSizing: 'border-box', padding: '20px', borderRadius: '20px', border: '1px solid rgba(255,176,32,.35)', background: '#0a0d0f' }}>
        <span style={{ fontFamily: 'var(--fm)', fontSize: '10.5px', letterSpacing: '.14em', textTransform: 'uppercase', color: '#ffb020' }}>
          Ventoinha e radiador contam
        </span>
        <p style={{ margin: '10px 0 0', fontSize: '15px', lineHeight: '23px', color: '#c9d0ca' }}>
          Radiador de water cooler na frente come de 25 a 55 mm do espaço da placa. A ficha do gabinete costuma dar o número com e sem ele.
        </p>
      </div>
