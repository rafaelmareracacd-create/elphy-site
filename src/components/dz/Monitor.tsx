...
import { contaMonitor, GR_MONITOR, INICIAL_MONITOR } from "@/lib/ferramentas/monitor";
import { useFerramenta } from "./useFerramenta";
import Link from "next/link";
import { Fragment } from "react";
import MenuBotao from "./Menu";

/* Transcrito de docs/design-1.0/Monitor-Mobile.dc.html */
export default function Monitor() {
  const { grupos, linhas, impresso, numero, cupom, data, rotulo, pre, mostrado, unidade, sub, nota, seloTopo, seloMeio, seloBase, seloFundo, ledCor, ledTexto, brilho, codigo, barras, mandarTexto, mandar } =
    useFerramenta(GR_MONITOR, INICIAL_MONITOR, contaMonitor, "/ferramentas/monitor/");
  return (
    <div className="dz-col" style={{ position: 'relative', width: '100%', minHeight: '1820px', overflow: 'hidden', display: 'flex', flexDirection: 'column', background: '#050607', color: '#eef2ee', fontFamily: 'var(--fb)' }}>
      ...
      <div style={{ padding: '28px 24px 0' }}>
        ...
        <p style={{ margin: '14px 0 0', fontSize: '16px', lineHeight: '24px', color: '#b9c1bb' }}>
          A caixa diz o tamanho e a resolução. A conta que junta os dois — e diz de que distância você vê o pixel — fica com você.
        </p>
      </div>
      {grupos.map((g, i) => (
        <Fragment key={i}>
      ...
      <div style={{ padding: '44px 24px 0', display: 'flex', flexDirection: 'column', gap: '10px' }}>
        ...
        {/* a ferramenta de telas de celular ainda não existe: fica marcada, como no menu */}
        <span aria-disabled="true" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', height: '56px', boxSizing: 'border-box', borderRadius: '28px', textDecoration: 'none', fontFamily: 'var(--fb)', fontWeight: '700', fontSize: '16px', marginTop: '0px', background: 'transparent', color: '#eef2ee', border: '1px solid rgba(255,255,255,.16)' , opacity: '.45' }}>
          Comparar telas de celular · em breve
        </span>
      </div>
      <div style={{ margin: '28px 24px 0', boxSizing: 'border-box', padding: '20px', borderRadius: '20px', border: '1px solid rgba(255,176,32,.35)', background: '#0a0d0f' }}>
        ...
        <p style={{ margin: '10px 0 0', fontSize: '15px', lineHeight: '23px', color: '#c9d0ca' }}>
          Meça a distância entre o seu olho e a tela na sua mesa. Se ela for menor que a da nota, você vai ver o pixel.
        </p>
      </div>
