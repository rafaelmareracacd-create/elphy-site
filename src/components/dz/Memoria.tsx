...
import { contaMemoria, GR_MEMORIA, INICIAL_MEMORIA } from "@/lib/ferramentas/memoria";
import { useFerramenta } from "./useFerramenta";
import Link from "next/link";
import { Fragment } from "react";
import MenuBotao from "./Menu";

/* Transcrito de docs/design-1.0/Memoria-Mobile.dc.html */
export default function Memoria() {
  const { grupos, linhas, impresso, numero, cupom, data, rotulo, pre, mostrado, unidade, sub, nota, seloTopo, seloMeio, seloBase, seloFundo, ledCor, ledTexto, brilho, codigo, barras, mandarTexto, mandar } =
    useFerramenta(GR_MEMORIA, INICIAL_MEMORIA, contaMemoria, "/ferramentas/memoria/");
  return (
    <div className="dz-col" style={{ position: 'relative', width: '100%', minHeight: '1700px', overflow: 'hidden', display: 'flex', flexDirection: 'column', background: '#050607', color: '#eef2ee', fontFamily: 'var(--fb)' }}>
      ...
      <div style={{ padding: '28px 24px 0' }}>
        ...
        <p style={{ margin: '14px 0 0', fontSize: '16px', lineHeight: '24px', color: '#b9c1bb' }}>
          Toca na geração do processador. A nota diz o tipo de memória e o teto oficial de velocidade.
        </p>
      </div>
      {grupos.map((g, i) => (
        <Fragment key={i}>
      ...
      <div style={{ margin: '28px 24px 0', boxSizing: 'border-box', padding: '20px', borderRadius: '20px', border: '1px solid rgba(255,176,32,.35)', background: '#0a0d0f' }}>
        <span style={{ fontFamily: 'var(--fm)', fontSize: '10.5px', letterSpacing: '.14em', textTransform: 'uppercase', color: '#ffb020' }}>
          DDR4 e DDR5 não se misturam
        </span>
        <p style={{ margin: '10px 0 0', fontSize: '15px', lineHeight: '23px', color: '#c9d0ca' }}>
          O chanfro do pente fica em lugar diferente: DDR5 não entra em slot de DDR4, nem ao contrário. Confira qual a placa-mãe usa antes de comprar.
        </p>
      </div>
